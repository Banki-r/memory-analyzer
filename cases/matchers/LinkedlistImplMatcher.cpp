...
private:
  StatementMatcher _cMatcher =
      integerLiteral(hasParent(cxxConstructorDecl().bind("init")));
  StatementMatcher _dwhileMatcher =
      whileStmt(hasDescendant(memberExpr().bind("whileVar")),
                hasCondition(binaryOperator().bind("whileOp")),
                hasAncestor(compoundStmt(hasAncestor(cxxDestructorDecl()))))
          .bind("whileStmt");
  StatementMatcher _dforMatcher =
      forStmt(hasDescendant(memberExpr().bind("forVar")),
...
public:
  virtual void run(const MatchFinder::MatchResult &result) override {
    ...
    if (deleteWhile && opWhile && whileVar) {
      if (opWhile->getOpcode() == clang::BinaryOperator::Opcode::BO_GT ||
          opWhile->getOpcode() == clang::BinaryOperator::Opcode::BO_GE ||
          opWhile->getOpcode() == clang::BinaryOperator::Opcode::BO_LT ||
          opWhile->getOpcode() == clang::BinaryOperator::Opcode::BO_LE) {
        for (auto iter = _ImplementedLinkedlists.begin();
             iter != _ImplementedLinkedlists.end(); ++iter) {
          if (iter->sizeVarName ==
              whileVar->getMemberNameInfo().getAsString()) {
            iter->deletedProperly = true;
          }
        }
      }
    } else if (deleteFor && opFor && forVar) {
      if (opFor->getOpcode() == clang::BinaryOperator::Opcode::BO_GT ||
          opFor->getOpcode() == clang::BinaryOperator::Opcode::BO_GE ||
          opFor->getOpcode() == clang::BinaryOperator::Opcode::BO_LT ||
          opFor->getOpcode() == clang::BinaryOperator::Opcode::BO_LE) {
        for (auto iter = _ImplementedLinkedlists.begin();
             iter != _ImplementedLinkedlists.end(); ++iter) {
